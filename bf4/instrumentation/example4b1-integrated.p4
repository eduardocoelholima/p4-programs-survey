...
struct flow_def_t2_0 {
    ...
    @matchKind("exact") 
    bool                         key_t2_0_hdr_ipv4_2__valid_;
}
...
struct flow_def_t3_0 {
    ...
    bit<8>                       set_ttl2__newTtl;
    @matchKind("exact") 
    bit<32>                      key_t3_0_hdr_ipv4_srcAddr;
}

@controlled extern flow_def_t3_0 query_t3_0(@matchKind("exact") in bit<32> t3_0_hdr_ipv4_srcAddr);
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    flow_def_t3_0 tmp_4;
    apply {
        tmp_2 = query_t1_0(hdr.ipv4.isValid());
        t1 = tmp_2;
        if (t1.hit) {
        ...
        if (t1.action_run == flow_def_t1_0__action_type_t._drop) {
            ...
            if (t1.action_run == flow_def_t1_0__action_type_t.copy_ip_src) {
                angelic_assert(true);
        ...
        end_t1_0();
        tmp_3 = query_t2_0(hdr.ipv4_2.isValid());
        t2 = tmp_3;
        if (t2.hit) {
            key_match(hdr.ipv4_2.isValid() == t2.key_t2_0_hdr_ipv4_2__valid_);
        }
        if (t2.action_run == flow_def_t2_0__action_type_t._drop_3) {
            angelic_assert(true);
        }
        else {
            if (t2.action_run == flow_def_t2_0__action_type_t.use_H12) {
                angelic_assert(true);
                if (hdr.ipv4_2.isValid() && hdr.ipv4.isValid()) {
                    hdr.ipv4.srcAddr = hdr.ipv4_2.srcAddr;
                }
        ...
        end_t2_0();
        tmp_4 = query_t3_0(hdr.ipv4.srcAddr);
        t3 = tmp_4;
        if (t3.hit) {
            key_match(hdr.ipv4.srcAddr == t3.key_t3_0_hdr_ipv4_srcAddr);
            if (!hdr.ipv4.isValid()) {
                bug();
            }
        }
        if (t3.action_run == flow_def_t3_0__action_type_t.NoAction_5) {
            ...
            if (t3.action_run == flow_def_t3_0__action_type_t._drop_4) {
                ...
                if (t3.action_run == flow_def_t3_0__action_type_t.set_ttl2) {
                    angelic_assert(true);
                    if (hdr.ipv4_2.isValid()) {
                        hdr.ipv4_2.ttl = t3.set_ttl2__newTtl;
                    }
                    else {
                        bug();
                    }
